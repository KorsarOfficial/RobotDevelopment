...
device = {
    ...
    "sensor": c_int(), # дескриптор датчика тока
}
...
def initDevice():
    ...
    errCode = lib.RI_SDK_CreateModelComponent("sensor".encode(), "voltage_sensor".encode(), "ina219".encode(), device["sensor"], errTextC)
    if errCode != 0:
        return errCode, errTextC
    ...
    errCode = lib.RI_SDK_LinkVoltageSensorToController(device["sensor"], device["i2c"], c_uint8(0x41), errTextC)
    if errCode != 0:
        return errCode, errTextC
...
# RI_SDK_exec_ServoDrive_Turn_With_Info - Выполняет поворот сервопривода на заданный угол с заданной угловой скоростью,
# Во время выполнения выводит информацию полученную датчиком тока.
def RI_SDK_exec_ServoDrive_Turn_With_Info(device,sensor, angle, speed, errTextC):
    voltage = c_float() # Напряжение в цепи
    voltageShunt = c_float() # Напряжение на шунте
    current = c_float() # Сила тока
    power = c_float() # Мощность
    state = c_int() # Состояние сервопривода
    errCode = c_int

    # выполняем поворот на заданный угол,передаем дескриптор тела,угол,скорость и асинхронный режим работы
    errCode = lib.RI_SDK_exec_ServoDrive_Turn(device, angle, speed, c_bool(True), errTextC)
    if errCode != 0:
        return errCode, errTextC

   
    # Бесконечный цикл. Пока состояние сервопривода не будет равно 0 - выводим показатели. 
    while(True):
        # Получаем состояние сервопривода
        errCode = lib.RI_SDK_exec_ServoDrive_GetState(device, state, errTextC)
        if errCode != 0:
            return errCode, errTextC
        
        errCode = lib.RI_SDK_sensor_VoltageSensor_Sense(sensor, voltage, voltageShunt, current, power, errTextC);
        if errCode != 0:
            return errCode, errTextC

        print("--------------------------------\n");
        print("Voltage = ", voltage.value, "\nShunt = ", voltageShunt.value,"\nCurrent = ", current.value, "\nPower = ", power.value);
        print("================================================================\n");
        if bool(state) != True:
            break
...
def rotateBody(angle, speed):
    ...
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор тела,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["body"], device["sensor"], angle, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор клешни,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["clawRotate"],device["sensor"], 45, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC
...
def get():
    ...
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowR"],device["sensor"], arrowR_over_cube_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowL"], device["sensor"], arrowL_over_cube_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор клешни,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["claw"],device["sensor"], claw_unclenched_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowR"],device["sensor"], (arrowR_cube_position - arrowR_over_cube_position), speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowL"], device["sensor"], (arrowL_cube_position - arrowL_over_cube_position), speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор клешни,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["claw"], device["sensor"], (-1) * claw_unclenched_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowR"], device["sensor"], (-1) * arrowR_cube_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC

    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowL"], device["sensor"], (-1) * arrowL_cube_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC
...
def put():
    ...
    errCode = c_int
    # выполняем мерцание светодиодом,передаем дескриптор светодиода,3 параметра цвета(RGB),продолжительность,кол-во повторений и включаем асинхронный режим работы
    lib.RI_SDK_exec_RGB_LED_Flicker(device["led"], 0, 0, 255, 500, 0, c_bool(True), errTextC)
    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowR"], device["sensor"], arrowR_cube_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowL"], device["sensor"], arrowL_cube_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор клешни,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["claw"], device["sensor"], claw_unclenched_position, speed, errTextC)
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowR"], device["sensor"], (-1) * arrowR_cube_position, speed,
                                              errTextC)
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор стрелы,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["arrowL"], device["sensor"], (-1) * arrowL_cube_position, speed,
                                              errTextC)
    if errCode != 0:
        return errCode, errTextC
    # выполняем поворот на заданный угол,передаем дескриптор клешни,угол,скорость и асинхронный режим работы
    errCode = RI_SDK_exec_ServoDrive_Turn_With_Info(device["claw"], device["sensor"], (-1) * claw_unclenched_position, speed,
                                              errTextC)
    if errCode != 0:
        return errCode, errTextC
