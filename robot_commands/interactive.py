...
"""
Интерактивное управление роботом RM 001 через консоль.
Доступные команды:
- left: поворот влево
...
- home: вернуться в домашнюю позицию
...
try:
    from utils import load_risdk_library, setup_sdk_functions, decode_error, print_command_help
    from constants import *
except ImportError:
    from robot_commands.utils import load_risdk_library, setup_sdk_functions, decode_error, print_command_help
...
def init_device():
    ...
    try:
        ...
        print("Использование: interactive.py -d <i2c_adapter_name>")
        sys.exit(2)
...
def move_left():
    ...
    # Включаем мигание синим светодиодом при движении
...
def move_down():
    ...
    # Включаем мигание желтым светодиодом при движении
    errCode = lib.RI_SDK_exec_RGB_LED_FlashingWithFrequency(
        device["led"], 
        255, 255, 0,  # RGB: желтый
        5,  # частота мигания
        0, 
        c_bool(True), 
        errTextC
...
def open_claw():
    ...
    # Включаем мигание оранжевым светодиодом при движении
    errCode = lib.RI_SDK_exec_RGB_LED_FlashingWithFrequency(
        device["led"], 
        255, 165, 0,  # RGB: оранжевый
        5,  # частота мигания
        0, 
        c_bool(True), 
        errTextC
...
def process_command(command):
    ...
    Returns:
        bool: True, если нужно продолжать, False, если нужно завершить работу.
    """
    ...
    try:
        if command == CMD_LEFT:
            errCode, errTextC = move_left()
            if errCode != 0:
                print(f"Ошибка при повороте влево: {decode_error(errTextC)}")
                return True
            print("Робот успешно повернулся влево")
            
        elif command == CMD_RIGHT:
            errCode, errTextC = move_right()
            if errCode != 0:
                print(f"Ошибка при повороте вправо: {decode_error(errTextC)}")
                return True
            print("Робот успешно повернулся вправо")
            
        elif command == CMD_UP:
            errCode, errTextC = move_up()
            if errCode != 0:
                print(f"Ошибка при поднятии руки: {decode_error(errTextC)}")
                return True
            print("Робот успешно поднял руку")
            
        elif command == CMD_DOWN:
            errCode, errTextC = move_down()
            if errCode != 0:
                print(f"Ошибка при опускании руки: {decode_error(errTextC)}")
                return True
            print("Робот успешно опустил руку")
            
        elif command == CMD_OPEN:
            errCode, errTextC = open_claw()
            if errCode != 0:
                print(f"Ошибка при открытии клешни: {decode_error(errTextC)}")
                return True
            print("Робот успешно открыл клешню")
            
        elif command == CMD_CLOSE:
            errCode, errTextC = close_claw()
            if errCode != 0:
                print(f"Ошибка при закрытии клешни: {decode_error(errTextC)}")
                return True
            print("Робот успешно закрыл клешню")
            
        elif command == CMD_HOME:
            errCode, errTextC = start_position_all_servo()
            if errCode != 0:
                print(f"Ошибка при возврате в домашнюю позицию: {decode_error(errTextC)}")
                return True
            print("Робот успешно вернулся в домашнюю позицию")
...
def interactive_loop():
    """
    Интерактивный цикл управления роботом.
    """
    print("\nИнтерактивное управление роботом RM 001")
    print("Введите команду или 'help' для справки")
    
    # Переводим робота в начальное положение
    print("\nПеревод робота в начальное положение...")
    errCode, errTextC = start_position_all_servo()
    if errCode != 0:
        print(f"Ошибка при инициализации начальной позиции: {decode_error(errTextC)}")
        return
    
    print("Робот готов к управлению!\n")
    print_command_help()
...
def main():
    """
    Главная функция, запускающая интерактивное управление роботом.
    """
    print("Инициализация робота RM 001 для интерактивного управления...")
    
    try:
        # Инициализируем библиотеку и компоненты
        errCode, errTextC = init_device()
        if errCode != 0:
            print(f"Ошибка при инициализации устройства: {decode_error(errTextC)}")
            sys.exit(2)
        ...
        errCode, errTextC = destruct()
        if errCode != 0:
            print(f"Ошибка при освобождении ресурсов: {decode_error(errTextC)}")
